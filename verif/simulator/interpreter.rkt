...
(require
 "../syntax.rkt" "../value.rkt" "../environment.rkt" "../result.rkt"
...
(define (local-eval expr st env globals)
  (cond
    ...
    [(quote? expr)
     ...
     ;; domain; simulator code itself will not use this, just top-level
  ...
  (define (lift f)
    (lambda (args st globals)
      ...
      (result (result-value oracle-result)
              (state (state-simulator st) (result-state oracle-result)))))
...
(define (meta->environment metadata)
  ...
  (append
   (list
    ;; constructor
    (cons 'circuit-new (raw (lift (meta-new-zeroed metadata))))
    ;; step
