...
(require
 ...
 (only-in "../environment.rkt" make-assoc assoc-extend assoc-extend*)
 (only-in "../value.rkt" closure)
...
(define-syntax ($#%module-begin stx)
  (syntax-parse stx
    [(_ form ...)
     ...
     #:with make-interpreter (format-id stx "make-interpreter")
     #'(#%module-begin
        (process-structs form ...)
        (define global-bindings
          (let ([global-bindings (make-assoc)])
            (process-defines global-bindings form ...)))
        (define (make-interpreter oracle-api metadata)
          (define global-env (create-environment oracle-api metadata global-bindings))
          (lambda (expr st) (interpret expr st global-env)))
        (provide global-bindings make-interpreter))]))
